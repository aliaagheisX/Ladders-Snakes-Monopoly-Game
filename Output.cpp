...
#include "Input.h"
int xGarbage, yGarbage;//garbage to store point clicked
////////////////////////////////////////////////////////////////////////////////////////// 
...
void Output::CreatePlayModeToolBar() const
{
	...
	// draw the tool bar icons in any way you want.
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\one.jpg";
...
void Output::DrawPlayer(const CellPosition & cellPos, int playerNum, color playerColor) const
{
	...
	if (!cellPos.IsValidCell() || playerNum < 0 || playerNum >= MaxPlayerCount) { return; }
	//Done
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if (!(fromCell.IsValidCell() && toCell.IsValidCell()) || fromCell.HCell() != toCell.HCell() || fromCell.VCell() <= toCell.VCell()) { return; }
